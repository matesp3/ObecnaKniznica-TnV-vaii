...
<form class="needs-validation" method="post" action="<?= $link->url('bookItem.save') ?>" enctype="multipart/form-data">
    <div id="bookForm" class="container py-3 px-5 my-5 rounded-3">
        ...
        <h3 class="text-center mb-5"> <?= ($data ? 'Editovať' : 'Pridať') ?> knihu </h3>
        <div class="row mb-3">
            ...
            <div class="col-md-9">
                ...
                <span>
                        <i>
                            <small id="fInfo"
                                   class=<?= !$data ? 'normal' : (($data['errors'] ?? null) ? 'invalid' : 'valid') ?>>
                                <?php if ($data) : ?>
        ...
        <div class="row">
            ...
            <div class=" col-md-8">
                     <textarea class="form-control" id="descr" name="description"
                               placeholder="Priestor na zhodnotenie knihy"><?=
                         $data ? (($data['errors'] ?? null) ? $data['previousInputs']['description'] : $data['bookItem']?->getDescription()) : null ?></textarea>
            </div>
        </div>

        <div class="row justify-content-center align-items-center">
            <button id="btnSubmitBookItem" type="submit" class="btn border-2 text-center col-3 col-sm-2 col-lg-1 mt-5">
                Uložiť
            </button>
        </div>
    </div>
